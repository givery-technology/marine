...
ただし、これをチャレンジ毎に毎回実装するのはしんどいのでライブラリとして[track-judge](./track-judge/reference.md)というユーティリティライブラリが用意されています。
...
- 定義ファイルの拡張子は`.json`でなければならない
  - 変換後のファイルは同じディレクトリに`.json`を取り除いたファイル名で保存されるのでtrack-judgeを使用する場合はinputのファイル名は`.json`のないファイル名で指定します。
...
### cliチャレンジの場合の自動定義
[track-judge](./track-judge/reference.md)を使用して、テストケースを設定ファイルで定義している場合はその内容から自動的にdebug定義が作成されます。
